...
- Visual similarity verification: balanced and aggressive modes now compute an 8×8 perceptual hash for each referenced image pair and reject outputs that exceed per-mode Hamming-distance thresholds (balanced 16/64, aggressive 22/64). See `packages/core/src/visualSimilarity.ts`.
