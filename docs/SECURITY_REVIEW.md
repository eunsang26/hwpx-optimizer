...
| Electron 제한 | renderer는 CSP `connect-src 'none'`을 사용하고, `nodeIntegration: false`, `contextIsolation: true`, `sandbox: true`로 실행된다. |
...
- 배포 공지에는 버전, 배포일, 담당자, 변경 내역, SHA256 값을 포함한다.
- 임의 재배포와 외부 반출을 금지한다.
